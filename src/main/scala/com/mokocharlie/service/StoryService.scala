...
class StoryService(storyRepository: StoryRepository)(implicit val system: ActorSystem)
  ...
  def storyById(id: Long): ServiceResponse[Story] =
    dbExecute(storyRepository.storyById(id))
}
